...
(defn find-min-for-pair [pair, maps]
  (let [[start, length] pair]
    (->>
     (range start (+ start length))
     (map #(go-through-maps % maps))
     (reduce min)
  ))
)

(with-open [rdr (clojure.java.io/reader "assets/day05/test_input.txt")]
  (let [lines (line-seq rdr)
    ...
    ;; "Not working on big input :sadcat"
    (->>
     (partition 2 seeds)
     (map #(find-min-for-pair % maps))
     (apply min))))